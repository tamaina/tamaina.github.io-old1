...
$.getScript("https://cdnjs.cloudflare.com/ajax/libs/materialize/0.97.7/js/materialize.min.js", function(){
materializeq();
});
...
var html = '',bar = '<ul id="nav-mobile" class="right hide-on-med-and-down">', slideout = '<ul id="slide-out" class="side-nav">', foothtml = '<ul>';
$.getJSON("./data/Nav.json", function(data){
//console.log('NavBar');
//console.table(data);
html += '<div class="navbar-fixed"><nav class="navigationbar"><div class="nav-wrapper" style="margin-left:1em;padding-right:1em;"><a href="javascript:void(0)" class="brand-logo grey-text text-lighten-3 blogo">'
         + data.title
         + '</a>';
...
bar  += '</ul>';
slideout += '</ul>';
html += bar + slideout + '<a href="javascript:void(0)" data-activates="slide-out" class="button-collapse blue-grey-text text-darken-4"><i class="fa fa-bars" aria-hidden="true"></i></a></div></nav></div>';
foothtml += '</ul>';
...
function WriteAd(){
$.getJSON("../lib/ads/list.json", function(data){
//console.log('ad');
//console.table(data);
...
var ad_l = data.length, s = 0, m = 0, x = 0, y = 0 ;
  ...
  var adurl = "../lib/ads/" + ad.URL + ".png", html = "";
if(ad.Link){
  html += '<a href="'
        + ad.Link
        + '" class="adl"><img src="'
...
tthtml += '<div class="card-panel" style="padding: 1.75px;"><h3 style="margin-left:20px;">1日目スケジュール</h3>'
        + ttwrite( ProgsArray, TTDay1, 1, 3.8 ) + '</div>';

tthtml += '<div class="card-panel" style="padding: 1.75px;"><h3 style="margin-left:20px;">2日目スケジュール</h3>'
        + ttwrite( ProgsArray, TTDay2, 2, 3.8 ) + '</div>';

$(outputtargetidTT).html( tthtml );


tthtmlx += '<div style="padding: 1.75px;">'
        + ttwritex( ProgsArray, TTDay1, 1, 3 ) + '</div>';

tthtmlx += '<div style="padding: 1.75px;">'
        + ttwritex( ProgsArray, TTDay2, 2, 3 ) + '</div>';
...
function ttwritex( ProgsArray, TTArray, Day, keisu ){
var ttdhtml = '', restuntil = 0;
if( Day == 1 ){ var restfrom = 720; }
if( Day == 2 ){ var restfrom = 540; }
if(TTArray[0]){
for(x=0;x<TTArray.length;x++){
  ...
  ttdhtml += '<div style="height: '
              + restheight * keisu
              + 'px;" class="ttxrest"></div>';
  }
  var divheight = TTArray[x][2] * keisu;
  ttdhtml += '<div style="height:100%; width: '
              + divheight + 'px;';
  if( ProgsArray[i][4] != "" ){
  ttdhtml += ProgsArray[i][4]
  }
  ...
  ttdhtml += ProgsArray[i][3][n][2] + ':' + ProgsArray[i][3][n][3] + ' ～ ' + ProgsArray[i][3][n][4] + ':' + ProgsArray[i][3][n][5]
              + '</span><div class="ttxprog"><p class="tth3">'
              + ProgsArray[i][1]
              + '</p>';
  if(ProgsArray[i][3][n][0]){
  ttdhtml += '<p class="ttsub">' + ProgsArray[i][3][n][0] + '</p>';
